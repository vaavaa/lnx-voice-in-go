...
| **xdotool** | Перетаскивание оверлея мышью за область **вне** иконки микрофона (Linux/X11; см. **Требования**) |
...
- **Перетаскивание окна**: утилита **`xdotool`** в `PATH` (пакет `xdotool` в дистрибутиве). Перенос оверлея результатом перетаскивания выполняется через неё и рассчитан на **X11**; на **чистом Wayland** перетаскивание может не работать (ограничения окружения), при этом запись и остальной функционал сохраняются.

Пакеты разработки Ubuntu/Debian для стека X11/CGO (из репозитория); **`xdotool`** для перетаскивания окна можно поставить той же командой или отдельно:
...
Оверлей можно **перетаскивать** за любое место **кроме** иконки микрофона (по микрофону — только старт/останов записи). Нужен установленный **`xdotool`**; см. **Требования**.
